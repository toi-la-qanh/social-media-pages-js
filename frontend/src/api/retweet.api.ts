...
export default class RetweetApi extends BaseApi {
    ...
    store(post_id: string) {
        return super.post(`${url}/${post_id}/retweets`, {}, { lang: language.value });
    }
    ...
    destroy(post_id: string) {
        return super.delete(`${url}/${post_id}/retweets`, { lang: language.value });
    }
    ...
    getCountOfRetweets(post_id: string) {
        return super.get(`${url}/${post_id}/retweets/count`, { lang: language.value });
    }
    ...
    getRetweetPostsOfUser(user_id: string, current_page: number = 1, total_pages: number = 10) {
        return super.get(`${url}/users/${user_id}/retweets?current_page=${current_page}&total_pages=${total_pages}`, { lang: language.value });
    }
}